...
downloadBtn.addEventListener("click", () => {
  ...
  // Reorder so analytical columns come first
...
function renderKpis(kpi) {
  ...
  // If ground truth was found in the upload, swap the 4th tile to show match accuracy
...
const CHART_PALETTE = [
  "#2d5a3d",  // forest green (accent)
  "#2d7d9a",  // proficient blue
  "#c47a1a",  // developing amber
  "#1a7a3a",  // exemplary green
  "#7c5fa6",  // purple
  "#b5392a",  // below red
  "#6b6b64",  // text secondary
];
...
function renderTable(rows, columns) {
  ...
  // Surface analytical columns first, in this order
  const surfaced = ["predicted_claim_type", "actual_claim_type",
                    "match_predicted_vs_actual", "topic", "extracted_location"];
  const ordered = [
  ...
  body.innerHTML = rows.map(r => {
    return "<tr>" + ordered.map(c => {
      ...
      if (c === "predicted_claim_type") {
        return `<td class="col-pred" title="${safe}">${safe}</td>`;
      }
      if (c === "actual_claim_type") {
        return `<td class="col-actual" title="${safe}">${safe}</td>`;
      }
      if (c === "match_predicted_vs_actual") {
        const cls = v === "Y" ? "match-y" : (v === "N" ? "match-n" : "");
        return `<td class="col-match"><span class="${cls}">${safe || "—"}</span></td>`;
      }
      if (c === "extracted_location") {
        return `<td class="col-loc" title="${safe}">${safe}</td>`;
      }
